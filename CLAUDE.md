...
Hono 기반 3-레이어 구조가 표준입니다. Nuxt의 `server/` 구조와 동일합니다.
...
│   └── users.js
├── dao/                  # 데이터 접근 (DB 쿼리, KV 읽기/쓰기) — 여러 api에서 재조합
│   └── users.js
└── utils/
    └── response.js       # notFound / badRequest / unauthorized / serverError
```

### server/index.js — 안정 파일 (수정 불필요)

`_registry.js`가 모든 라우트를 자동 등록한다. 새 라우트 추가 시 이 파일을 편집하지 않는다.
...
### server/_registry.js — 자동 생성 파일

`npm run scan` 또는 Claude Code hook이 `server/api/` 폴더를 스캔하여 자동 생성한다. **직접 편집하지 않는다.**

```js
// AUTO-GENERATED by vue-zero scan — do not edit
import r0 from './api/auth.js'
import r1 from './api/users.js'

export default function register(app) {
  app.route('/api/auth', r0)
  app.route('/api/users', r1)
}
```

마운트 경로 규칙: `server/api/X.js` → `/api/X`, `server/api/admin/settings.js` → `/api/admin/settings`
...
### 규칙 1 — 파일 등록은 `npm run scan`이 자동 처리

`.vue` 파일이나 `server/api/*.js` 파일을 추가/삭제하면 `npm run scan`을 실행한다.
Claude Code 사용 시 hook이 자동으로 실행하므로 수동 실행 불필요.

- `pages/*.vue` → `pages/pages.json` 자동 동기화
- `components/*.vue` → `components/components.json` 자동 동기화
- `server/api/*.js` → `server/_registry.js` 자동 생성

```bash
npm run scan        # pages.json, components.json, _registry.js 모두 갱신
```
