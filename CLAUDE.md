...
├── routes/               # HTTP 엔드포인트 정의 — 파일 추가 시 scan이 자동 등록
│   ├── auth.js           # POST /api/auth/login, /api/auth/logout
│   └── users.js
├── dao/                  # 데이터 접근 (DB 쿼리, KV 읽기/쓰기) — 여러 routes에서 재조합
│   └── users.js
└── utils/
    └── response.js       # notFound / badRequest / unauthorized / serverError
```

### api/index.js — 안정 파일 (수정 불필요)
...
### api/_registry.js — 자동 생성 파일

`npm run scan` 또는 Claude Code hook이 `api/routes/` 폴더를 스캔하여 자동 생성한다. **직접 편집하지 않는다.**

```js
// AUTO-GENERATED by vue-zero scan — do not edit
import r0 from './routes/auth.js'
import r1 from './routes/users.js'
...
마운트 경로 규칙: `routes/X.js` → `/api/X`, `routes/admin/settings.js` → `/api/admin/settings`
...
`.vue` 파일이나 `api/routes/*.js` 파일을 추가/삭제하면 `npm run scan`을 실행한다.
...
- `api/routes/*.js` → `api/_registry.js` 자동 생성
